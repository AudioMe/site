			...
			// file
				...
				el.css({position: 'absolute', top: '-50%', right: '-50%', fontSize: '200px', opacity: 0}).each(function() {
					if (el.parent('span.jq-file').length < 1) {
						...
						var name = $('<div class="name" style="float: right; white-space: nowrap"></div>').appendTo(file);
				...
				el.each(function() {
					if (el.next('span.jqselect').length < 1) {
						...
						function selectbox() {
                            ...
                            //var icon = opt.icon ? "<span>1</span>" : "";
                            var icon = "<span class='icon_16'></span>";
							var list = '';
							...
							function makeList() {
								for (i = 0; i < option.length; i++) {
									...
									li = '<li class="' + liClass + optionClass + '">'+"<span class='"+option.eq(i).attr('class')+"'></span>"+ option.eq(i).text() +'</li>';
									// если есть optgroup
							...
							function doSelect() {
								...
								if (optionSelected.length) {
									divText.text(optionSelected.text()).prepend("<span class='"+optionSelected.attr('class')+"'></span>");
								} else {
									divText.text(option.first().text()).prepend("<span class='"+option.first().attr('class')+"'></span>");
								}
								...
								// если селект активный
									...
									// изменение селекта
									el.change(function() {
										divText.text(option.filter(':selected').text()).prepend("<span class='"+option.filter(':selected').attr('class')+"'></span>");
