...
<div class="clearfix">
    ...
    <div class="article_date">дата публикации <?php echo $news->date;?></div>
        <a href="/news" class="ui_button arrow">Назад к новостям</a>
    </div>
    <div class="article_body">
        <?php 
        if (isset($images) && isset($images[0])) {
        $image = $images[count($images)-1];?>
        <img class="float_l" src="/files/<?php echo $news->user_id . "/news/prev_" . $image->id . ".png"; ?>" class="float_l" />
        <?php } ?>
        <h3><?php echo str_replace('&nbsp;', ' ', $news->preview);?></h3>
        <?php
