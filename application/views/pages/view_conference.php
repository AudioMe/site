...
<div class="clearfix">
    ...
    <h2><?php if (isset($conference->title)) echo $conference->title;?></h2>
    <div class="article_date">дата публикации <?php echo $conference->date;?></div>
        <a href="/conference" class="ui_button arrow">Назад</a>
    </div>
    <div class="article_body">
        <?php 
        if (isset($images) && isset($images[0])) {
        $image = $images[count($images)-1];?>
        <img class="float_l" src="/files/<?php echo $conference->user_id . "/conference/prev_" . $image->id . ".png"; ?>" class="float_l" />
        <?php } ?>
        <h3><?php echo str_replace('&nbsp;', ' ', $conference->preview);?></h3>
        <?php
        if (isset($conference->text) && $conference->text != "") { 
            $text = html_entity_decode($conference->text);
            echo $text;
        }?>
    </div>
</div>
...
<div class="share_block clearfix">
    ...
    <div class="share42init" data-title="<?php if (isset($conference->title)) echo $conference->title; ?>"></div>
</div>
<?php
$stud = $studios->where('user_id', '=', $conference->user_id)->find();
if (isset($stud->id)) { ?>
    ...
    <div class="tab1 clearfix">
        ...
        <div id="comment-form" class="comment-form tab1">
            ...
            <div id="respond" class="tab1 p_comments_block">
                <form action="/addFree/commentsConference/<?php if (isset($conference->id)) echo $conference->id; ?>"
